...
## 先说结论：OVHcloud 是当前可用的免注册例外

[OVHcloud AI Endpoints 官方文档](https://docs.ovhcloud.com/en/guides/public-cloud/ai-machine-learning/ai-endpoints-capabilities)明确写出匿名层：每个 IP、每个模型每分钟 2 次；同一平台也提供视觉模型和 OpenAI 兼容接口。[官方入门页](https://docs.ovhcloud.com/en/guides/public-cloud/ai-machine-learning/ai-endpoints-getting-started)列出可处理图像的 Qwen VL 模型。插件默认使用：
...
空的 `visionApiKeyEnv` 是有意设计：插件不读取凭据，直接使用 OVHcloud 匿名层。当前默认适合低频视觉请求；超过 2 RPM、需要更稳定配额或要使用项目级限额时，再创建自己的 OVHcloud Access Token。官方文档确认匿名速率限制，但没有对所有地区和场景作“永久免费”或商业用途承诺，正式使用前仍应查看当前条款。

这不代表所有“免费 API”都免注册。当前没有发现第二个同时满足“不注册、不用 Key、有稳定 HTTPS OpenAI 兼容视觉接口”的可靠公开服务；下面提到的其他网站不能作为本插件后端。
...
[Pollinations API](https://github.com/pollinations/pollinations/blob/main/APIDOCS.md) 虽然提供视觉能力，但官方 API 文档要求从 `enter.pollinations.ai` 获取 API Key；[讯飞图像理解](https://www.xfyun.cn/doc/spark/ImageUnderstanding.html) 使用带应用凭据的签名 WebSocket，也不是免注册 OpenAI 兼容端点。因此，除 OVHcloud 官方匿名层外，“免注册”目前只能作为人工网页试用选项，不能作为本插件的即开即用后端。

## 推荐顺序

| 选项 | 需要注册 | 视觉模型/入口 | 适合谁 |
| --- | --- | --- | --- |
| OVHcloud AI Endpoints | 不需要（匿名层） | `Qwen2.5-VL-72B-Instruct`，OpenAI 兼容 | 首次安装、低频使用、想即开即用的人 |
...
## 1. OVHcloud AI Endpoints（默认，免注册）

无需注册或申请 Key，直接安装插件即可。匿名层限制为每个 IP、每个模型每分钟 2 次；VLM 请求体上限为 10 MB，具体限制以[官方能力说明](https://docs.ovhcloud.com/en/guides/public-cloud/ai-machine-learning/ai-endpoints-capabilities)为准。
...
如需认证限额，在 OVHcloud Public Cloud 项目中创建 Access Token，然后覆盖配置：
...
PowerShell 使用 `$env:OVH_AI_ENDPOINTS_ACCESS_TOKEN = '<你的 OVHcloud Token>'`；POSIX shell 使用 `export OVH_AI_ENDPOINTS_ACCESS_TOKEN='<你的 OVHcloud Token>'`。Token 是可选项，不要把它写入 patch 或仓库。
...
- OVHcloud 匿名层遇到 `429` 时，等待下一个分钟窗口，或切换到自己的 Access Token。
